...
export default function Violations() {
  ...
  const handlePay = async (v) => {
    if ((currentUser.walletBalance || 0) < v.fineAmount) { alert('Insufficient wallet balance. Please add funds first.'); return; }
    await payFine(v);
  };
  const handleCheckOverstays = async () => {
    const count = await triggerOverstays();
    alert(`Overstay check complete. ${count} new violation(s) found.`);
  };
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="space-y-12 pb-24">
      <motion.div variants={itemAnim} className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <button onClick={handleCheckOverstays} className="px-8 py-5 font-display font-black text-sm uppercase tracking-widest rounded-[1.5rem] flex items-center gap-3 transition-all shadow-aesthetic dark:bg-v3-slate/40 dark:border-white/5 dark:hover:bg-v3-ruby/10" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5DFD7' }}
          onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#FDF2F2'; }}
          onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#FFFFFF'; }}
        >
          <AlertTriangle size={20} className="text-v3-ruby" /> Check Overstays
        </button>
      </motion.div>
      ...
      <motion.div variants={itemAnim} className="glass-panel rounded-[3rem] overflow-hidden">
        <div className="p-8 bg-v3-ruby/5 dark:bg-v3-ruby/10 flex items-center gap-4" style={{ borderBottom: '1px solid rgba(225,29,72,0.1)' }}>
          ...
          <span className="text-sm font-bold text-v3-ruby">Overstaying a reservation incurs an automatic 100 PKR + 50 PKR/hr fine.</span>
        </div>
            ...
            <motion.div key={v.id} variants={itemAnim} className="flex flex-col md:flex-row justify-between items-center p-8 rounded-[2rem] gap-6 dark:border-white/5 dark:bg-black/20" style={{ border: '1px solid #E5DFD7', backgroundColor: 'rgba(255,255,255,0.4)' }}>
              ...
              <div className="flex items-center gap-8">
                ...
                {v.isPaid ? (
                  ...
                  <button onClick={() => handlePay(v)} className="px-8 py-4 bg-v3-ruby text-white font-display font-black text-sm rounded-[1.5rem] hover:scale-105 active:scale-95 transition-all shadow-xl">Pay Fine</button>
                )}
              </div>
            </motion.div>
          ))}
